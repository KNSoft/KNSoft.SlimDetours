...
内联挂钩需要修改函数开头的指令以实现跳转。为应对有线程正好运行在待修改指令上的可能，需要更新处于此状态的线程，避免其执行非法的新旧指令混合。

## 其它挂钩库中的实现

### Detours

[Detours](https://github.com/microsoft/Detours)提供了[`DetourUpdateThread`](https://github.com/microsoft/Detours/wiki/DetourUpdateThread)函数更新线程，但需要由调用方传入需要更新的线程句柄：
```C
...
但[Detours](https://github.com/microsoft/Detours)对线程的更新在x64下仍有遗漏之处，参考我为此提交的[PR #344: Improve thread program counter adjustment](https://github.com/microsoft/Detours/pull/344)。

> [!TIP]
> 虽然它的官方示例“[Using Detours](https://github.com/microsoft/Detours/wiki/Using-Detours)”中有`DetourUpdateThread(GetCurrentThread())`这样的代码，但这种用法无意义且无效。应使用它更新进程中除当前线程外的所有线程，详见[`DetourUpdateThread`](https://github.com/microsoft/Detours/wiki/DetourUpdateThread)。但即便以正确的方式更新线程，也会带来新的风险，见[🔗 技术Wiki：更新线程时避免堆死锁](https://github.com/KNSoft/KNSoft.SlimDetours/blob/main/Docs/TechWiki/Avoid%20Heap%20Deadlocks%20When%20Updating%20Threads/README.zh-CN.md)。
